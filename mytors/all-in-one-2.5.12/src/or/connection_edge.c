...
if (get_options()->ClientOnly == 1){
	  if (begin_type == RELAY_COMMAND_BEGIN){
		...
		tor_log(LOG_NOTICE, LD_CONTROL,"@iSec:%s:ASpath:%s:%s:%u:%s:%u:%s:%u:%s:%f,%f:%d:%ld:%d:%d:%u:Done",get_options()->MYIP,get_options()->MYIP, circ->base_.entry_ip, circ->base_.gbw,circ->base_.middle_ip,circ->base_.mbw,circ->base_.exit_ip, circ->base_.ebw,ap_conn->socks_request->address,circ->base_.pop_lat,circ->base_.pop_lon,circ->base_.rand_no, get_mean_list(TO_CIRCUIT(circ), 1),edge_conn->stream_id, circ->base_.rand_no, (unsigned)circ->base_.n_circ_id);
  }
}
//iSec--------------------------------------------------

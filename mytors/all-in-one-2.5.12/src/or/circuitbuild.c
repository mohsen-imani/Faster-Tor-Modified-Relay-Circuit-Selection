...
extern int NUM_BUILT_CIRC;
extern int NUM_ATTACHED_CIRC;
int CIRCUIT_COUNTER_ISEC = 0; 
static int
onion_pick_cpath_exit_isec(origin_circuit_t *circ, extend_info_t *exit, float lat, float lon, float Mylat, float Mylon);
static int
onion_populate_cpath_isec(origin_circuit_t *circ, float Mylat, float Mylon);
static int
onion_extend_cpath_isec(origin_circuit_t *circ, float lat, float lon);
static const node_t *
choose_good_exit_server_isec(uint8_t purpose,
                        cpath_build_state_t *state,int need_uptime, int need_capacity, int is_internal, float lat, float lon, float Mylat, float Mylon);

static const node_t *
choose_good_exit_server_general_isec(int need_uptime, int need_capacity, float lat, float lon, float Mylat, float Mylon);


static const node_t *
choose_good_middle_server_isec(uint8_t purpose,
...
circuit_establish_circuit(uint8_t purpose, extend_info_t *exit, int flags)
{
 ...
 //iSec:-------------------------------------------------
  if (circ && circ->build_state->is_internal == 0 && circ->build_state->onehop_tunnel == 0 && get_options()->ClientOnly == 1){
     tor_log(LOG_NOTICE, LD_CONTROL,"@iSec:%s:ALL-IN-ONE-CREATE:%s:%f,%f:%u:%s:%f,%f:%u:%s:%f,%f:%u:%f,%f:%d:%u:Done",get_options()->MYIP,circ->base_.entry_ip, circ->base_.glat,circ->base_.glon,circ->base_.gbw,circ->base_.middle_ip, circ->base_.mlat,circ->base_.mlon, circ->base_.mbw,circ->base_.exit_ip,circ->base_.elat,circ->base_.elon, circ->base_.ebw,circ->base_.pop_lat, circ->base_.pop_lon,  circ->base_.rand_no, circ->base_.n_circ_id);
	  NUM_BUILT_CIRC ++;
  }
 //iSec:-------------------------------------------------
  return circ;
...
circuit_send_next_onion_skin(origin_circuit_t *circ)
{
  ...
  if (circ->cpath->state == CPATH_STATE_CLOSED) {
    ...
    if (!hop) {
     ...
     //isec------------------------------------------------------------
     if ((get_options()->VANILLA == 4) && (get_options()->ClientOnly == 1) && !can_complete_circuit && !circ->build_state->onehop_tunnel && (circ->base_.purpose == CIRCUIT_PURPOSE_C_GENERAL)) {
	CIRCUIT_COUNTER_ISEC ++;
	log_info(LD_CIRC, "We built one of our circuits (%d id:%d)",CIRCUIT_COUNTER_ISEC,circ->base_.rand_no);
     }

     if ((get_options()->VANILLA == 4) && (get_options()->ClientOnly == 1) && (CIRCUIT_COUNTER_ISEC == get_options()->OPEN_CIRCUITS_BOOTSTRAP_ISEC)) {
     ...
     if ((get_options()->VANILLA != 4) || (get_options()->ClientOnly != 1)){
	      if (!can_complete_circuit && !circ->build_state->onehop_tunnel) {
		const or_options_t *options = get_options();
		can_complete_circuit=1;
		/* FFFF Log a count of known routers here */
...
//                 try, router->nickname);
  ...
  tor_free(n_supported);
  if (node) {
    log_info(LD_CIRC, "@iSec:%s:Chosen-exit-server:%f:%f:%s", get_options()->MYIP, node->lat, node->lon, node_describe(node));
    return node;
  }
  if (options->ExitNodes) {
    log_warn(LD_CIRC,
...
onion_pick_cpath_exit(origin_circuit_t *circ, extend_info_t *exit)
{
  ...
  if (state->onehop_tunnel) {
    log_debug(LD_CIRC, "@iSec:%s: Launching a one-hop circuit for dir tunnel.", get_options()->MYIP);
  ...
  log_debug(LD_CIRC, "@iSec:%s:circ len:%d:circ MYID:%d", get_options()->MYIP,state->desired_path_len,circ->base_.rand_no);//
  if (exit) { /* the circuit-builder pre-requested one */
    warn_if_last_router_excluded(circ, exit);
    log_info(LD_CIRC,"@iSec:%s:Using requested exit node '%s'", get_options()->MYIP,
...
//iSec:-------------------------------------------------------
   ...
   GeoIPRecord *gir;
   gir = GeoIP_record_by_name(gi, circ->base_.exit_ip);
   if (gir != NULL) {
	circ->base_.elat =  gir->latitude;
	circ->base_.elon =  gir->longitude;
   }else{
	circ->base_.elat =  get_options()->MYLAT;
	circ->base_.elon =  get_options()->MYLON;
   }
   if (gir)
...
log_info(LD_CIRC,"@iSec:%s:selected exit:%s:lat:%f:lon:%f:Done",get_options()->MYIP, circ->base_.exit_ip,circ->base_.elat,circ->base_.elon);
...
onion_extend_cpath(origin_circuit_t *circ)
{
  ...
  //iSec:-------------------------------------------------------------
   ...
   GeoIPRecord *gir;
   gir = GeoIP_record_by_name(gi, circ->base_.entry_ip);
   if (gir != NULL) {
	circ->base_.glat =  gir->latitude;
	circ->base_.glon =  gir->longitude;
   }else{
	circ->base_.glat =  get_options()->MYLAT;
	circ->base_.glon =  get_options()->MYLON;
   }
   if (gir)
      GeoIPRecord_delete(gir);
   if (gi)
      GeoIP_delete(gi);
    log_info(LD_CIRC,"@iSec:%s:ENTRY:%s:COORD:%f:%f",get_options()->MYIP, circ->base_.entry_ip,circ->base_.glat,circ->base_.glon);
  //iSec:-------------------------------------------------------------
    }
  } else {
    const node_t *r =
  ...
  //iSec:-------------------------------------------------------------
   ...
   GeoIPRecord *gir;
   gir = GeoIP_record_by_name(gi, circ->base_.middle_ip);
   if (gir != NULL) {
	circ->base_.mlat =  gir->latitude;
	circ->base_.mlon =  gir->longitude;
   }else{
	circ->base_.mlat =  get_options()->MYLAT;
	circ->base_.mlon =  get_options()->MYLON;
   }
   if (gir)
      GeoIPRecord_delete(gir);
   if (gi)
...
//#############################################################
origin_circuit_t *
circuit_establish_circuit_isec(uint8_t purpose, extend_info_t *exit, int flags,float lat, float lon, float Mylat, float Mylon)
{
  origin_circuit_t *circ;
  int err_reason = 0;
  circ = origin_circuit_init(purpose, flags);
  circ->base_.pop_lat = lat;//
  circ->base_.pop_lon = lon;//
  if (onion_pick_cpath_exit_isec(circ, exit, lat,lon,Mylat,Mylon) < 0 ||//
      onion_populate_cpath_isec(circ,Mylat,Mylon) < 0) {
    circuit_mark_for_close(TO_CIRCUIT(circ), END_CIRC_REASON_NOPATH);
    return NULL;
  }
...
onion_pick_cpath_exit_isec(origin_circuit_t *circ, extend_info_t *exit, float lat, float lon, float Mylat, float Mylon)
{
  ...
  if (exit) { /* the circuit-builder pre-requested one */
    ...
    const node_t *node =
      choose_good_exit_server_isec(circ->base_.purpose, state ,state->need_uptime,
...
#define MAX_POPULATE_ATTEMPTS 32
  ...
  while (1) {
    int r = onion_extend_cpath_isec(circ, Mylat, Mylon);
    if (r < 0) {
      log_info(LD_CIRC,"Generating cpath hop failed.");
      return -1;
    }
...
onion_extend_cpath_isec(origin_circuit_t *circ, float lat, float lon)
{
  ...
  if (cur_len == state->desired_path_len - 1) { /* Picking last node */
    ...
    const node_t *r = choose_good_entry_server_isec(purpose, state, lat,lon);
    if (r) {
      /* If we're a client, use the preferred address rather than the
         primary address, for potentially connecting to an IPv6 OR
         port. */
...
//iSec:-------------------------------------------------------
    ...
    const node_t *r =
      choose_good_middle_server_isec(purpose, state, circ->cpath, cur_len);
    if (r) {
      info = extend_info_from_node(r, 0);
      tor_assert(info);
...
//iSec:-------------------------------------------------------
   ...
   if (gi)
    ...
    log_info(LD_CIRC,"@iSec:%s:MIDDLE:%s:COORD:%f:%f",get_options()->MYIP, circ->base_.middle_ip,circ->base_.mlat,circ->base_.mlon);
//iSec:-------------------------------------------------------
    }
  }
...
//#############################################################
static const node_t *
choose_good_exit_server_isec(uint8_t purpose, cpath_build_state_t *state,
                        int need_uptime, int need_capacity, int is_internal, float lat, float lon, float Mylat, float Mylon)
{

  const node_t *r = NULL;
  const or_options_t *options = get_options();
  if (state && options->UseEntryGuards && (options->NumEntryGuards == 1) &&
      (purpose != CIRCUIT_PURPOSE_TESTING || options->BridgeRelay)) {
    /* This request is for an entry server to use for a regular circuit,
     * and we use entry guard nodes.  Just return one of the guard nodes.  */
    r = choose_random_entry(state);
  }
  ...
  if (r){
   ...
   log_info(LD_CIRC, "@iSec:%s: We changed the start point from (%f,%f) to (%f,%f)", get_options()->MYIP, Mylat, Mylon, tmp_lat,tmp_lon);
  router_crn_flags_t flags = CRN_NEED_DESC;
  if (need_uptime)
    flags |= CRN_NEED_UPTIME;
  if (need_capacity)
  ...
  switch (purpose) {
    case CIRCUIT_PURPOSE_C_GENERAL:
      if (options->AllowInvalid_ & ALLOW_INVALID_MIDDLE)
        flags |= CRN_ALLOW_INVALID;
      if (is_internal) /* pick it like a middle hop */
        return router_choose_random_node_isec(NULL, options->ExcludeNodes, flags, tmp_lat,tmp_lon,lat,lon);
      else
        return choose_good_exit_server_general_isec(need_uptime,need_capacity, lat,lon,tmp_lat,tmp_lon);
    case CIRCUIT_PURPOSE_C_ESTABLISH_REND:
      if (options->AllowInvalid_ & ALLOW_INVALID_RENDEZVOUS)
        flags |= CRN_ALLOW_INVALID;
      return router_choose_random_node(NULL, options->ExcludeNodes, flags);
...
choose_good_exit_server_general_isec(int need_uptime, int need_capacity, float lat, float lon, float Mylat, float Mylon)
{
  log_info(LD_CIRC, "@iSec:%s:EXIT-SELECTION:BETWEEN:%f:%f:AND:%f:%f:DONE", get_options()->MYIP, Mylat, Mylon, lat,lon);
  int *n_supported;
  int n_pending_connections = 0;
  smartlist_t *connections;
  int best_support = -1;
...
//             router->nickname);
  ...
  if (best_support > 0) {
    ...
    node = node_sl_choose_by_bandwidth_isec(supporting, WEIGHT_FOR_EXIT,Mylat,Mylon,lat,lon);//iSec
    smartlist_free(supporting);
  } else {
    /* Either there are no pending connections, or no routers even seem to
     * possibly support any of them.  Choose a router at random that satisfies
    ...
    if (best_support == -1) {
      if (need_uptime || need_capacity) {
        log_info(LD_CIRC,
                 ...
                 need_uptime?", stable":"");
        tor_free(n_supported);
        return choose_good_exit_server_general_isec(0, 0, lat, lon,Mylat,Mylon);// iSec
      }
      log_notice(LD_CIRC, "All routers are down or won't exit%s -- "
                 "choosing a doomed exit at random.",
                 options->ExcludeExitNodesUnion_ ? " or are Excluded" : "");
...
choose_good_entry_server_isec(uint8_t purpose, cpath_build_state_t *state, float lat, float lon)
{
  ...
  router_crn_flags_t flags = CRN_NEED_GUARD|CRN_NEED_DESC;
  const node_t *node;
  float exit_lat = close_dest_lat;//iSec
  float exit_lon = close_dest_lon;//iSec
  log_info(LD_CIRC, "@iSec:%s:ENTRY-SELECTION:BETWEEN:%f:%f:AND:%f:%f:DONE", get_options()->MYIP, lat,lon,exit_lat,exit_lon);
  ...
  choice = router_choose_random_node_isec(excluded, options->ExcludeNodes, flags,lat,lon,exit_lat,exit_lon);
  smartlist_free(excluded);
  return choice;
}
                          ...
                          int cur_len)
{
  ...
  for (i = 0, cpath = head; i < cur_len; ++i, cpath=cpath->next) {
    ...
    log_info(LD_CIRC, "@iSec:%s:MIDDLE-SELECTION:BETWEEN:%f:%f:AND:%f:%f:DONE", get_options()->MYIP,entry_lat,entry_lon,exit_lat,exit_lon);
  if (state->need_uptime)
    flags |= CRN_NEED_UPTIME;
  if (state->need_capacity)
    flags |= CRN_NEED_CAPACITY;
  if (options->AllowInvalid_ & ALLOW_INVALID_MIDDLE)
    flags |= CRN_ALLOW_INVALID;
  choice = router_choose_random_node_isec(excluded, options->ExcludeNodes, flags,entry_lat,entry_lon,exit_lat,exit_lon);
  smartlist_free(excluded);
  return choice;
}

...
//iSec----------------------------------------------------------------------------
	if ((get_options()->ClientOnly == 1) && (get_options()->VANILLA == 0)){
		node = choose_good_entry_server_isec(CIRCUIT_PURPOSE_C_GENERAL, NULL, get_options()->MYLAT, get_options()->MYLON);
	}else{
...
tor_log(LOG_NOTICE, LD_CONTROL, "@iSec:%s:NEW-ENTRY-GUARD:%d:%s:%f:%f:NICKNAME:%s",get_options()->MYIP, for_directory,node->node_ip,node->lat,node->lon,
           node_describe(node));
                         ...
                         dirinfo_type_t dirinfo_type, int *n_options_out)
{
  ...
  const int num_needed = decide_num_guards(options, for_directory);
  log_info(LD_CIRC, "@iSec:%s:NUM-GUARDS:%d:DIR?:%d", get_options()->MYIP, num_needed,for_directory);
  if (n_options_out)
    *n_options_out = 0;

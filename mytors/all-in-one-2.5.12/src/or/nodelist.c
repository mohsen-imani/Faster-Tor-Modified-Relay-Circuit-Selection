...
nodelist_set_consensus(networkstatus_t *ns)
{
  ...
  SMARTLIST_FOREACH_BEGIN(ns->routerstatus_list, routerstatus_t *, rs) {
    ...
    //iSec:-------------------------------------------
    log_info(LD_CIRC,"@iSec:%s:Reading the nodes' geo information from database",get_options()->MYIP);
    ...
    gir  = GeoIP_record_by_name(gi, node->node_ip);
    if (gir != NULL) {
		  node->lat =  gir->latitude;
		  node->lon =  gir->longitude;
    }else{
		  node->lat =  (float)get_options()->MYLAT;
		  node->lon =  (float)get_options()->MYLON;
    }
	//--------------------------------------------- 

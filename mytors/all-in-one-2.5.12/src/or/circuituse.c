...
#include <GeoIP.h>//iSec
#include <GeoIPCity.h>//iSec
#include "main.h"
//iSec:--------------------------------
int global_circ_cntr_isec = 0;
static origin_circuit_t *
circuit_get_best_vanilla(const entry_connection_t *conn,
...
circuit_get_best_car(const entry_connection_t *conn,
                 ...
                 int need_uptime, int need_internal);
static origin_circuit_t *
circuit_get_best_all_in_one(const entry_connection_t *conn,
                 int must_be_open, uint8_t purpose,
                 int need_uptime, int need_internal);
static origin_circuit_t *
circuit_get_best_evaluation(const entry_connection_t *conn,
                 int must_be_open, uint8_t purpose,
                 int need_uptime, int need_internal);
void circuit_stream_is_being_handled_isec2();
//iSec:--------------------------------
...
//iSec:---------------------------------------------
      if ((get_options()->VANILLA == 0 || get_options()->VANILLA == 2) && (get_mean_list(circ_, 5)> (get_options()->CUTOFF)) && ((get_options()->ClientOnly == 1) && !need_internal && purpose == CIRCUIT_PURPOSE_C_GENERAL && !(origin_circ->build_state->onehop_tunnel) && !conn->want_onehop)){
	log_info(LD_CIRC,"@iSec:%s:IGNORED:NOT ACCEPTED:DONE",get_options()->MYIP);
        return 0;
      }	
//iSec:---------------------------------------------
  return 1;
                 ...
                 int need_uptime, int need_internal)
{
   ...
   if ((get_options()->VANILLA == 0) && (get_options()->ClientOnly == 1) && (!need_internal && (purpose == CIRCUIT_PURPOSE_C_GENERAL) && !conn->want_onehop)){
	//all-in-all in clinets
	return circuit_get_best_all_in_one(conn,must_be_open,purpose,need_uptime,need_internal);
   }else if (((get_options()->VANILLA == 3) || (get_options()->VANILLA == 4))&& (get_options()->ClientOnly == 1) && (!need_internal && (purpose == CIRCUIT_PURPOSE_C_GENERAL) && !conn->want_onehop)){
	//CAR in clinets
	return circuit_get_best_evaluation(conn,must_be_open,purpose,need_uptime,need_internal);
   }else if ((get_options()->VANILLA == 2) && (get_options()->ClientOnly == 1) && (!need_internal && (purpose == CIRCUIT_PURPOSE_C_GENERAL) && !conn->want_onehop)){
	//evaluating circuit selection in clinets
	return circuit_get_best_car(conn,must_be_open,purpose,need_uptime,need_internal);
   }else if ((get_options()->VANILLA == 1)){
	//vanilla in clinets
	return circuit_get_best_vanilla(conn,must_be_open,purpose,need_uptime,need_internal);
   }else{
	//otheres in clinets
	return circuit_get_best_vanilla(conn,must_be_open,purpose,need_uptime,need_internal);
  }
...
circuit_remove_handled_ports(smartlist_t *needed_ports)
{
  ...
  for (i = 0; i < smartlist_len(needed_ports); ++i) {
    ...
    if (circuit_stream_is_being_handled(NULL, *port,
                                        get_options()->OPEN_CIRCUITS_PER_PORT_ISEC)) {
  ...
  TOR_LIST_FOREACH(circ, circuit_get_global_list(), head) {
    if (CIRCUIT_IS_ORIGIN(circ) &&
      ...
      if (origin_circ->unusable_for_new_conns)
        continue;
    //iSec:------------------------------------------------
    // Throw away congested circuits in both OURS and CAR
    if ((get_options()->ClientOnly == 1) && ((get_options()->VANILLA == 0))){
	    if (get_mean_list(circ, 5)> (get_options()->CUTOFF) && !build_state->is_internal){//
	      tor_log(LOG_NOTICE, LD_CONTROL,"@iSec:%s:IGNORED-CONGESTED-IN-HANDLER:%d:CONGESTED:%ld:DONE", get_options()->MYIP, circ->rand_no,get_mean_list(circ, 5));
	      continue;
	    }
    }
    //iSec:-------------------------------------------------
...
circuit_predict_and_launch_new(void)
{
  ...
  TOR_LIST_FOREACH(circ, circuit_get_global_list(), head) {
    ...
    if ((circ->timestamp_dirty) && !((get_options()->ClientOnly == 1) && (get_options()->VANILLA == 4)))
    ...
    //iSec:-------------------------------------------------
    // skip if we are in our desing or CAR
    if ((get_options()->ClientOnly == 1) && ((get_options()->VANILLA == 0))){
	    if (get_mean_list(circ, 5)> (get_options()->CUTOFF) && !build_state->is_internal){//
	      tor_log(LOG_NOTICE, LD_CONTROL,"@iSec:%s:IGNORED-CONGESTED:%d:CONGESTED:%ld:DONE", get_options()->MYIP, circ->rand_no,get_mean_list(circ, 5));
	      //circ->marked_for_close = 1;
	      //detach_reattach_streams_moh(circ);
	      continue;
	    }
    }
    after_cong++;
  ...
  tor_log(LOG_NOTICE, LD_CONTROL,
		     "@iSec:%s:CLEAN-CIRCUITS:%d:CLEANED-CONGESTED::%d:DONE",get_options()->MYIP,num, after_cong);//




  int mycntr = 0;
  if (get_options()->ClientOnly == 1 && get_options()->MYLOG == 1){
	  TOR_LIST_FOREACH(circ, circuit_get_global_list(), head) {
	    int is_dirty, is_internal, is_onehop, dirtiness;
	    cpath_build_state_t *build_state;
	    origin_circuit_t *origin_circ;
	    if (!CIRCUIT_IS_ORIGIN(circ))
	      continue;
	    ...
	    is_dirty = 0;
	    if (circ->timestamp_dirty)
	      is_dirty  = 1; /* only count clean circs */
	    //if (circ->purpose != CIRCUIT_PURPOSE_C_GENERAL)
	    origin_circ = TO_ORIGIN_CIRCUIT(circ);
	    is_onehop = 0;
	    build_state = origin_circ->build_state;
	    ...
	    if (build_state->is_internal)
	      is_internal = 1;
	    dirtiness = 0;
	    if (circ->timestamp_dirty && (circ->timestamp_dirty + get_options()->MaxCircuitDirtiness < now))
		dirtiness = 1;
	    mycntr++;
	   tor_log(LOG_NOTICE, LD_CONTROL,"iSec|IP|%s|%d|CIRC_COUNTER|%d|INT|%d|OH|%d|P|%s|DIRTY|%d|ID|%u|MYID|%d|EXP|%d|LEN|%d|T|%ld|DONE",get_options()->MYIP,global_circ_cntr_isec,mycntr,is_internal, is_onehop,circuit_purpose_to_string(circ->purpose),is_dirty,circ->n_circ_id, circ->rand_no, dirtiness, build_state->desired_path_len,(long)(now - circ->timestamp_dirty));

	  }
  }
  global_circ_cntr_isec++;
  ...
  if (num >= MAX_UNUSED_OPEN_CIRCUITS)
    ...
    //iSec:-------------------------------------------------
    // Here we add our design and circuits to populare destinations
    if ((get_options()->ClientOnly == 1) && (get_options()->VANILLA == 0)){
  	  smartlist_t *dest = rep_hist_get_predicted_destination(); 
  	  SMARTLIST_FOREACH_BEGIN(dest, predicted_destination *, pd) {
	     if (circuit_stream_is_being_handled_isec(pd->lat, pd->lon) < get_options()->MIN_NUMBER_OF_CIRC_FOR_EACH_DEST){ 
  	     log_info(LD_CIRC,
		     "@iSec:%s:CREAT-CIRC-FOR-POP:%f:%f:DONE",get_options()->MYIP, pd->lat,pd->lon);//
             flags |= CIRCLAUNCH_NEED_UPTIME;
	     flags |= CIRCLAUNCH_NEED_CAPACITY;
	     circuit_launch_isec(CIRCUIT_PURPOSE_C_GENERAL, flags, pd->lat,pd->lon, get_options()->MYLAT, get_options()->MYLON);   
             }   
    ...
    if ((get_options()->ClientOnly == 1) && (get_options()->VANILLA == 4)){
         circuit_stream_is_being_handled_isec2();
	  if (num < get_options()->OPEN_CIRCUITS_ISEC){
		  int i = num;
		  for (i = num; i < get_options()->OPEN_CIRCUITS_ISEC;i++){
			    if (port_needs_uptime)
			      flags |= CIRCLAUNCH_NEED_UPTIME;
			    if (port_needs_capacity)
			      flags |= CIRCLAUNCH_NEED_CAPACITY;
			    log_notice(LD_GENERAL,
				     "We are building circuits because we do not have enough circuits (%d/%d)",i - num + 1,get_options()->OPEN_CIRCUITS_ISEC - num);
  ...
  if (   (((get_options()->VANILLA == 0) || (get_options()->VANILLA == 4)) && (get_options()->ClientOnly == 0))    || ((get_options()->VANILLA != 0) && (get_options()->VANILLA != 4)) ){
  ...
  //iSec: circuit build times are disabled in config.c
                                ...
                                origin_circuit_t **circp)
{
  ...
  need_uptime = !conn->want_onehop && !conn->use_begindir &&
                smartlist_contains_int_as_string(options->LongLivedPorts,
                                          conn->socks_request->port);
  log_debug(LD_CIRC, "@iSec:%s:LOOK-FOR-CIRC:STR:%d:beginDir:%d addrs:%s, %s",get_options()->MYIP,conn->want_onehop,conn->use_begindir,conn->socks_request->address,ENTRY_TO_CONN(conn)->address);
...
//iSec:-----------------------------------------------------
      log_info(LD_CIRC, "@iSec:%s:We want to build circuit for this connection", get_options()->MYIP);


      if ((get_options()->ClientOnly == 1 && get_options()->VANILLA == 0)){
          float my_lat = (float)get_options()->MYLAT;
          float my_lon = (float)get_options()->MYLON;
	  tor_addr_t addr_moh;
	  float lat = my_lat;//destination's coordinate
	  float lon = my_lon;//destination's coordinate
          float min_dis = 100000000;
          float min_lat = 0, min_lon = 0;

	  if (tor_addr_parse(&addr_moh, conn->socks_request->address)>0){
	     GeoIP *gi = GeoIP_open(get_options()->GeoPath, GEOIP_INDEX_CACHE);
	     GeoIPRecord *gir = GeoIP_record_by_name(gi, conn->socks_request->address);
	     if (gir){
		lat = gir->latitude;
		lon = gir->longitude;
	     }
  	  ...
  	  smartlist_t *dest = rep_hist_get_predicted_destination(); 
  	  SMARTLIST_FOREACH_BEGIN(dest, predicted_destination *, pd) {
             if (min_dis > compute_distance(pd->lat,pd->lon, lat, lon)){
		min_lat = pd->lat;
		min_lon = pd->lon;
                min_dis = compute_distance(pd->lat,pd->lon, lat, lon);
             }   
 	  } SMARTLIST_FOREACH_END(pd);
          smartlist_free(dest);
          tor_log(LOG_NOTICE, LD_CONTROL,"@iSec:%s:CIRC-CREAT-FOR:FR)M:%f:%f:FOR:%f:%f:DONE", get_options()->MYIP,min_lat, min_lon, lat, lon);
          circ = circuit_launch_by_extend_info_isec(new_circ_purpose, extend_info,
                                           flags,min_lat, min_lon, my_lat, my_lon);
...
connection_ap_handshake_attach_circuit(entry_connection_t *conn)
{
  ...
  if (!connection_edge_is_rendezvous_stream(ENTRY_TO_EDGE_CONN(conn))) {
    ...
    if (retval < 1) // XXX023 if we totally fail, this still returns 0 -RD
      return retval;
   if (!want_onehop)  
   tor_log(LOG_NOTICE, LD_CONTROL,"@iSec:%s:STREAM-ATTACHED:%s:TO:%u:%d:DONE", get_options()->MYIP,conn->socks_request->address,(unsigned)circ->base_.n_circ_id, circ->base_.rand_no);
...
//##################################################################
origin_circuit_t *
circuit_launch_isec(uint8_t purpose, int flags, float lat, float lon, float Mylat, float Mylon)
{
  origin_circuit_t *circ = circuit_launch_by_extend_info_isec(purpose, NULL, flags, lat, lon, Mylat, Mylon);
  log_info(LD_REND|LD_CIRC,"@iSec:%s:LAUNCHED CIRFC:ID:%u:entry:%s:middle:%s:exit:%s:DONE", get_options()->MYIP,circ->base_.n_circ_id,circ->base_.entry_ip, circ->base_.middle_ip, circ->base_.exit_ip);
  return circ;
}

//##################################################################
origin_circuit_t *
circuit_launch_by_extend_info_isec(uint8_t purpose,
                              extend_info_t *extend_info,
...
//    log_fn(LOG_INFO,"%d failures so far, not trying.",n_circuit_failures);
  ...
  /* try a circ. if it fails, circuit_mark_for_close will increment
   * n_circuit_failures */
  return circuit_establish_circuit_isec(purpose, extend_info, flags,lat, lon, Mylat, Mylon);
}


//##################################################################
int
circuit_stream_is_being_handled_isec(float lat, float lon)
{
  circuit_t *circ;
  int num=0;
  time_t now = time(NULL);
  ...
  TOR_LIST_FOREACH(circ, circuit_get_global_list(), head) {
    if (CIRCUIT_IS_ORIGIN(circ) &&
      ...
      if (!circ->timestamp_dirty && ((circ->timestamp_created.tv_sec + (60.0 * get_options()->CIRC_LIFE_TIME_ISEC)) < now)) {
	circuit_mark_for_close(circ, END_CIRC_AT_ORIGIN);
	tor_log(LOG_NOTICE, LD_CONTROL,"@iSec:%s:The circuit mark for close created at %d and died at %d, life time was %d sec", get_options()->MYIP, circ->timestamp_created.tv_sec, now, now - circ->timestamp_created.tv_sec);
	continue;
      }
...
//Mohsen---------------------------------------------
      if ((((circ->pop_lat - lat) < 0.1) && ((circ->pop_lat - lat) > -0.1)) && (((circ->pop_lon - lon) > -0.1) && ((circ->pop_lon - lon) < 0.1))){
	num++;
        }
      }
  }
                 ...
                 int need_uptime, int need_internal)
{
  ...
  if ((get_options()->ClientOnly == 1) && (!need_internal && (purpose == CIRCUIT_PURPOSE_C_GENERAL) && !conn->want_onehop)){
	int mycntr = 0;
	TOR_LIST_FOREACH(circ, circuit_get_global_list(), head) { ////SMARTLIST_FOREACH_END(circ);
		if (TO_ORIGIN_CIRCUIT(circ)->build_state->onehop_tunnel == 1)
			continue;
	        double di = compute_distance(my_lat, my_lon, circ->glat, circ->glon) + 
		compute_distance(circ->glat, circ->glon, circ->mlat, circ->mlon) +
		compute_distance(circ->mlat, circ->mlon, circ->elat, circ->elon) +
		compute_distance(circ->elat, circ->elon, lat, lon);
		float av = (circ->gbw + circ->ebw + circ->mbw)/3.0;
		if (purpose == CIRCUIT_PURPOSE_C_GENERAL){
			mycntr++;
		}
		log_info(LD_REND|LD_CIRC,"@iSec:%s:ALL-IN-ONE-COUNT:%d:%u:%d:%d:%d:%s:%d:%d:%s:%f,%f:%u:%s:%f,%f:%u:%s:%f,%f:%u:%f,%f:%f:%f:%ld:%ld:Done",get_options()->MYIP,mycntr, circ->n_circ_id,circ->rand_no, TO_ORIGIN_CIRCUIT(circ)->has_opened,TO_ORIGIN_CIRCUIT(circ)->build_state->onehop_tunnel,get_options()->MYIP,get_options()->MYLAT,get_options()->MYLON, circ->entry_ip, circ->glat,circ->glon,circ->gbw,circ->middle_ip, circ->mlat,circ->mlon, circ->mbw,circ->exit_ip,circ->elat,circ->elon, circ->ebw,circ->pop_lat, circ->pop_lon, di,av,get_mean_list(circ, 1),get_mean_list(circ, 5));

	} //SMARTLIST_FOREACH_END(circ);
  tor_log(LOG_NOTICE, LD_CONTROL,"@iSec:%s:CIRCUITS-AVAILABLE:%d:DONE", get_options()->MYIP,mycntr);
  }
                 ...
                 int need_uptime, int need_internal)
{
		...
		return circuit_get_best_vanilla(conn, must_be_open, purpose, need_uptime, need_internal);
		  ...
		  //SMARTLIST_FOREACH_END(circ);
		   tor_log(LOG_NOTICE, LD_CONTROL,"@iSec:%s:CIRCUITS-AVAILABLE:%d:DONE", get_options()->MYIP,cntr);
		  ...
		  for (j = 0; j < len ; j++ ){
			...
			if (get_options()->VANILLA == 4 && get_options()->CIRC_SELECT == 3){//RTT
				tmp = get_mean_list(circ, 1);
			}else{// congestion
				tmp = get_mean_list(circ, 5);
			}

			log_info(LD_REND|LD_CIRC,"@iSec:%s:TOP-CIRC:MYID:%d:entry:%s:middle:%s:exit:%s:CONG:%ld:POP:%f:%f:DEST:%f:%f:DONE", get_options()->MYIP, circ->rand_no,circ->entry_ip, circ->middle_ip, circ->exit_ip,tmp, circ->pop_lat,circ->pop_lon, lat,lon);
			if (tmp < min){
				min = tmp;
...
/*
CIRC_SELECT == 1 Circ Selection: Length Then Congestion
	       3 Circ Selection: RTT Only
    	       7 only length
	       2 Congestion Then Length
	       4 Congestion only
	       5 RTT then length
	       6 length then RTT
*/
	  ...
	  //SMARTLIST_FOREACH_END(circ);
	  if (cntr < 1){
		log_info(LD_REND|LD_CIRC,"@iSec:%s: Wow We Got less than 1 Circuits", get_options()->MYIP);
		return NULL;
	  }	

	  log_info(LD_REND|LD_CIRC,"@iSec:%s: Number of suitable circs: %d", get_options()->MYIP,cntr);

	     if (get_options()->CIRC_SELECT == 1){ // length then congestion	
		  log_info(LD_REND|LD_CIRC,"@iSec:%s: Circ Selection: Length Then Congestion ", get_options()->MYIP);  

		  //---------------------------------------------------
		  my_bucket *bucket  = tor_malloc_zero(sizeof(my_bucket)*cntr);
		  if (bucket == NULL){
			free(bucket);
			return NULL;
		  }
		  if (cntr > get_options()->CIRC_NEEDED)
			k = get_options()->CIRC_NEEDED;
		  else
			k = cntr;
		  // Now put all circuit ids in bucket------------------------------------
		  i = 0;
		  TOR_LIST_FOREACH(circ, circuit_get_global_list(), head) { ////SMARTLIST_FOREACH_END(circ);
		    origin_circuit_t *origin_circ;
		    if (!CIRCUIT_IS_ORIGIN(circ))
		    ...
		    bucket[i].ID = circ->rand_no;
		    bucket[i].distance = compute_distance(my_lat, my_lon, circ->glat, circ->glon) + 
			compute_distance(circ->glat, circ->glon, circ->mlat, circ->mlon) +
			compute_distance(circ->mlat, circ->mlon, circ->elat, circ->elon) +
			compute_distance(circ->elat, circ->elon, lat, lon);
		   i++;
		  ...
		  qsort (bucket, cntr , sizeof(my_bucket), compare_d); // sorting
		  for (j = 0; j < cntr ; j++ ){
			log_info(LD_REND|LD_CIRC,"@iSec:%s:CIRC-LIST:MYID:%d:LEN:%f",get_options()->MYIP, bucket[j].ID, bucket[j].distance);
		  }

		  for (j = 0; j < k ; j++ ){
			circ = circuit_get_by_id(bucket[j].ID);
			tmp = get_mean_list(circ, 5);
			log_info(LD_REND|LD_CIRC,"@iSec:%s:TOP-CIRC:MYID:%d:entry:%s:middle:%s:exit:%s:CONG:%ld:LEN:%f:POP:%f:%f:DEST:%f:%f:DONE", get_options()->MYIP, circ->rand_no,circ->entry_ip, circ->middle_ip, circ->exit_ip, tmp, bucket[j].distance, circ->pop_lat,circ->pop_lon, lat,lon);
			if (tmp < min){
				min = tmp;
			        best = TO_ORIGIN_CIRCUIT(circ);
			}
	     ...
	     }else if (get_options()->CIRC_SELECT == 3){  

		  log_info(LD_REND|LD_CIRC,"@iSec:%s: Circ Selection: RTT Only", get_options()->MYIP);  
			  ...
			  TOR_LIST_FOREACH(circ, circuit_get_global_list(), head) { ////SMARTLIST_FOREACH_END(circ);
				...
				compute_distance(circ->mlat, circ->mlon, circ->elat, circ->elon) +
				compute_distance(circ->elat, circ->elon, lat, lon);*/
			   bucket[i].distance = (float)get_mean_list(circ, 1); //long to float
			   i++;
			  }
			  //SMARTLIST_FOREACH_END(circ);
			  ...
			  for (j = 0; j < cntr ; j++ ){
			log_info(LD_REND|LD_CIRC,"@iSec:%s:CIRC-LIST:MYID:%d:RTT:%f",get_options()->MYIP, bucket[j].ID, bucket[j].distance);
			  }

			  for (j = 0; j < cntr ; j++ ){
				circ = circuit_get_by_id(bucket[j].ID);
				tmp = (long) bucket[j].distance;
				log_info(LD_REND|LD_CIRC,"@iSec:%s:TOP-CIRC:MYID:%d:entry:%s:middle:%s:exit:%s:RTT:%ld:POP:%f:%f:DEST:%f:%f:DONE", get_options()->MYIP, circ->rand_no,circ->entry_ip, circ->middle_ip, circ->exit_ip, tmp, circ->pop_lat,circ->pop_lon, lat,lon);
				if (tmp < min){
					min = tmp;
					best = TO_ORIGIN_CIRCUIT(circ);
				}
		  ...
		  //-------------------------------------------------------
		  ...
		  }else if (get_options()->CIRC_SELECT == 7){

		  	  log_info(LD_REND|LD_CIRC,"@iSec:%s: Circ Selection: Only Length", get_options()->MYIP);  
			  ...
			  TOR_LIST_FOREACH(circ, circuit_get_global_list(), head) { ////SMARTLIST_FOREACH_END(circ);
			    ...
			    tmp = (long)compute_distance(my_lat, my_lon, circ->glat, circ->glon) + 
				compute_distance(circ->glat, circ->glon, circ->mlat, circ->mlon) +
				compute_distance(circ->mlat, circ->mlon, circ->elat, circ->elon) +
				compute_distance(circ->elat, circ->elon, lat, lon);
			    log_info(LD_REND|LD_CIRC,"@iSec:%s:CIRC-LIST:MYID:%d:entry:%s:middle:%s:exit:%s:LEN:%ld:RTT:%f:POP:%f:%f:DEST:%f:%f:DONE", get_options()->MYIP, circ->rand_no,circ->entry_ip, circ->middle_ip, circ->exit_ip, tmp, bucket[i].distance, circ->pop_lat,circ->pop_lon, lat,lon);
				if (tmp < min){
					min = tmp;
					best = TO_ORIGIN_CIRCUIT(circ);
				}
		  ...
		  }else if (get_options()->CIRC_SELECT == 2){

		  	  log_info(LD_REND|LD_CIRC,"@iSec:%s: Circ Selection: Congestion Then Length", get_options()->MYIP);  
			  ...
			  TOR_LIST_FOREACH(circ, circuit_get_global_list(), head) { ////SMARTLIST_FOREACH_END(circ);
			    ...
			    bucket[i].ID = circ->rand_no;
			    bucket[i].distance = (float)get_mean_list(circ, 5); //long to float
			   i++;
			  }
			  //SMARTLIST_FOREACH_END(circ);
			  ...
			  qsort (bucket, cntr , sizeof(my_bucket), compare_d); // sorting
			  for (j = 0; j < cntr ; j++ ){
			      log_info(LD_REND|LD_CIRC,"@iSec:%s:CIRC-LIST:MYID:%d:CONG:%f",get_options()->MYIP, bucket[j].ID, bucket[j].distance);
			  }
			  if (get_options()->OPTIMAL == 1)
				 k = cntr;
			  for (j = 0; j < k ; j++ ){
				circ = circuit_get_by_id(bucket[j].ID);
				tmp = (long)compute_distance(my_lat, my_lon, circ->glat, circ->glon) + 
				...
				compute_distance(circ->mlat, circ->mlon, circ->elat, circ->elon) +
				compute_distance(circ->elat, circ->elon, lat, lon);
				log_info(LD_REND|LD_CIRC,"@iSec:%s:TOP-CIRC:MYID:%d:entry:%s:middle:%s:exit:%s:LEN:%ld:CONG:%f:POP:%f:%f:DEST:%f:%f:DONE", get_options()->MYIP, circ->rand_no,circ->entry_ip, circ->middle_ip, circ->exit_ip, tmp, bucket[j].distance, circ->pop_lat,circ->pop_lon, lat,lon);
				if (tmp < min){
					min = tmp;
					best = TO_ORIGIN_CIRCUIT(circ);
				}
		  ...
		  //-------------------------------------------------------
		    } else if (get_options()->CIRC_SELECT == 4){// just closest dest

		        log_info(LD_REND|LD_CIRC,"@iSec:%s: Circ Selection: congestion Only", get_options()->MYIP);  
			 my_bucket *bucket  = tor_malloc_zero(sizeof(my_bucket)*cntr);
			  if (bucket == NULL){
				free(bucket);
				return NULL;
		  ...
		  //-------------------------------------------------------
		  }else if (get_options()->CIRC_SELECT == 5){

		  	  log_info(LD_REND|LD_CIRC,"@iSec:%s: Circ Selection: RTT Then Length", get_options()->MYIP);  
		  ...
		  //-------------------------------------------------------
		  }else if (get_options()->CIRC_SELECT == 8){

		  	  log_info(LD_REND|LD_CIRC,"@iSec:%s: Circ Selection: RTT Then Congestion", get_options()->MYIP);  
		  ...
		  }else if (get_options()->CIRC_SELECT == 9){

		  	  log_info(LD_REND|LD_CIRC,"@iSec:%s: Circ Selection: Congestion Then RTT", get_options()->MYIP);  
	      ...
	      }else if (get_options()->CIRC_SELECT == 6){ // length then congestion	
		  log_info(LD_REND|LD_CIRC,"@iSec:%s: Circ Selection: Length Then RTT ", get_options()->MYIP);  
	...
	return NULL;
     ...
     if (get_options()->CIRC_SELECT == 1){ // length then congestion	
	  ...
	  //-------------------------------------------------------
	  	  ...
	  	  log_info(LD_REND|LD_CIRC,"@iSec:%s: Circ Selection: Closest Pop Dest", get_options()->MYIP);  

	      // So far we computed the lat and lon of our destination server. 
		  double min_dstnc = 999999999;
		  double dst = 0.0;
		  float pop_lat = my_lat;
		  float pop_lon = my_lon;
		  //---------------------------------------------------
		  TOR_LIST_FOREACH(circ, circuit_get_global_list(), head) { ////SMARTLIST_FOREACH_END(circ);
		    origin_circuit_t *origin_circ;
		    if (!CIRCUIT_IS_ORIGIN(circ))
		    ...
		    if (!circuit_is_acceptable(origin_circ,conn,must_be_open,purpose,
				               need_uptime,need_internal,now.tv_sec))
		      continue;
			dst = compute_distance(lat, lon,circ->pop_lat, circ->pop_lon);
		        log_info(LD_REND|LD_CIRC,"@iSec:%s:CIRC-LIST:MYID:%d:entry:%s:middle:%s:exit:%s:LEN:%f:POP:%f:%f:DEST:%f:%f:DONE", get_options()->MYIP, circ->rand_no,circ->entry_ip, circ->middle_ip, circ->exit_ip, dst, circ->pop_lat,circ->pop_lon, lat,lon);
			if (dst <= min_dstnc){
			min_dstnc = dst;
			pop_lat = circ->pop_lat;
			pop_lon = circ->pop_lon;
			//best = origin_circ;
			}
		    /* now this is an acceptable circ to hand back. but that doesn't
		     * mean it's the *best* circ to hand back. try to decide.
		  ...
		  TOR_LIST_FOREACH(circ, circuit_get_global_list(), head) { ////SMARTLIST_FOREACH_END(circ);
		    ...
		    if (!circuit_is_acceptable(origin_circ,conn,must_be_open,purpose,
				               need_uptime,need_internal,now.tv_sec))
		      continue;
			dst = compute_distance(my_lat, my_lon, circ->glat, circ->glon) + 
			compute_distance(circ->glat, circ->glon, circ->mlat, circ->mlon) +
			compute_distance(circ->mlat, circ->mlon, circ->elat, circ->elon) +
			compute_distance(circ->elat, circ->elon, lat, lon);
		        log_info(LD_REND|LD_CIRC,"@iSec:%s:CIRC-LIST:MYID:%d:entry:%s:middle:%s:exit:%s:LEN:%f:POP:%f:%f:DEST:%f:%f:DONE", get_options()->MYIP, circ->rand_no,circ->entry_ip, circ->middle_ip, circ->exit_ip, dst, circ->pop_lat,circ->pop_lon, lat,lon);
			if ((dst <= min_dstnc) && ((pop_lat - circ->pop_lat) < 0.1) && ((pop_lat - circ->pop_lat) > -0.1)){
			min_dstnc = dst;
			best = origin_circ;
			}
		    /* now this is an acceptable circ to hand back. but that doesn't
	  ...
	  //-------------------------------------------------------
	    ...
	    } else if (get_options()->CIRC_SELECT == 6){
	  	  log_info(LD_REND|LD_CIRC,"@iSec:%s: Circ Selection: SMALL RTT AMONG TOP SHORT CIRCs to TWO closest POPs", get_options()->MYIP);  

		  //---------------------------------------------------
		  double dist1 = 99999999;
		  double dist2 = 99999999;
		  double dist = 0.0;
	  	  smartlist_t *dest = rep_hist_get_predicted_destination(); 
	  	  SMARTLIST_FOREACH_BEGIN(dest, predicted_destination *, pd) {

			dist = 	compute_distance(lat, lon,pd->lat, pd->lon);
			log_info(LD_REND|LD_CIRC,"@iSec:%s:FIND-2nd-CLOSE-POP:DIST:%f:POP:%f:%f:DEST:%f:%f:DONE",get_options()->MYIP ,dist,pd->lat, pd->lon, lat,lon);
			if (dist < dist1){
				dist2 = dist1;
				dist1 = dist;
			}else if (dist < dist2){
		  ...
		  if (smartlist_len(dest) <= 2)
			dist2 = dist1;
		  smartlist_free(dest);
		  log_info(LD_REND|LD_CIRC,"@iSec:%s:2nd-SMALLEST-DIST::DIST2:%f:DIST1:%f:DEST:%f:%f:DONE", get_options()->MYIP ,dist2, dist1,lat,lon);
		  ...
		  TOR_LIST_FOREACH(circ, circuit_get_global_list(), head) { ////SMARTLIST_FOREACH_END(circ);
		    ...
		    bucket[i].ID = circ->rand_no;
		    bucket[i].distance = (float)compute_distance(lat, lon,circ->pop_lat, circ->pop_lon);//(float)get_mean_list(circ, 5); //long to float
		        log_info(LD_REND|LD_CIRC,"@iSec:%s:CIRC-LIST:MYID:%d:entry:%s:middle:%s:exit:%s:DIST:%f:POP:%f:%f:DEST:%f:%f:DONE", get_options()->MYIP, circ->rand_no,circ->entry_ip, circ->middle_ip, circ->exit_ip, bucket[i].distance, circ->pop_lat,circ->pop_lon, lat,lon);
		  ...
		  while (t < 2){
			  for (j = 0; j < cntr ; j++ ){
				circ = circuit_get_by_id(bucket[j].ID);
				tmp = (long)get_mean_list(circ, 1);

		        	log_info(LD_REND|LD_CIRC,"@iSec:%s:TOP-CIRC:MYID:%d:entry:%s:middle:%s:exit:%s:RTT:%ld:LEN:%f:POP:%f:%f:DEST:%f:%f:DONE", get_options()->MYIP, circ->rand_no,circ->entry_ip, circ->middle_ip, circ->exit_ip, tmp, bucket[j].distance, circ->pop_lat,circ->pop_lon, lat,lon);



				if (bucket[j].distance <= (dist2 + 10.0)){
     ...
     }else if (get_options()->CIRC_SELECT == 5){ // length then congestion	
	  log_info(LD_REND|LD_CIRC,"@iSec:%s: Circ Selection: SMALL RTT AMONG TOP 2 SHORT CIRCs", get_options()->MYIP);  
	  ...
	  TOR_LIST_FOREACH(circ, circuit_get_global_list(), head) { ////SMARTLIST_FOREACH_END(circ);
		...
		compute_distance(circ->mlat, circ->mlon, circ->elat, circ->elon) +
		compute_distance(circ->elat, circ->elon, lat, lon);
	        log_info(LD_REND|LD_CIRC,"@iSec:%s:CIRC-INFO:MYID:%d:entry:%s:middle:%s:exit:%s:LEN:%f:POP:%f:%f:DEST:%f:%f:DONE", get_options()->MYIP, circ->rand_no,circ->entry_ip, circ->middle_ip, circ->exit_ip, bucket[i].distance, circ->pop_lat,circ->pop_lon, lat,lon);
	   i++;
	  }
	  //SMARTLIST_FOREACH_END(circ);	

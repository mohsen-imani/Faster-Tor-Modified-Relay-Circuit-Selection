...
//iSec----------------------------------------------
  srand(time(NULL)); //iSec 
  predicted_destination_init();//iSec
  NUM_BUILT_CIRC = 0;
  NUM_ATTACHED_CIRC = 0;
  tor_log(LOG_NOTICE, LD_CONTROL, "@iSec:%s: lat %d lon %d and my IP is %s", get_options()->MYIP,get_options()->MYLAT, get_options()->MYLON, get_options()->MYIP);
  smartlist_t *dest = rep_hist_get_predicted_destination(); 
  double tmp_moh = 9999999999;
  double dist = 0.0;
  SMARTLIST_FOREACH_BEGIN(dest, predicted_destination *, pd) {
  	  tor_log(LOG_NOTICE, LD_CONTROL,"@iSec:%s: pop destinations lat %f lon %f", get_options()->MYIP,pd->lat,pd->lon);//Mohsen
	  dist = compute_distance(get_options()->MYLAT, get_options()->MYLON,pd->lat,pd->lon);
	  if (dist < tmp_moh){
	 	close_dest_lat = pd->lat;
		close_dest_lon = pd->lon;
		tmp_moh = dist;
	  }
		
  } SMARTLIST_FOREACH_END(pd);
	tor_log(LOG_NOTICE, LD_CONTROL,"@iSec:%s: The closest destination lat %f lon %f", get_options()->MYIP,close_dest_lat,close_dest_lon);


	log_debug(LD_CIRC, "@iSec:%s:CONFIG:CIRC_NEEDED:%d:", get_options()->MYIP, get_options()->CIRC_NEEDED);
	log_debug(LD_CIRC, "@iSec:%s:CONFIG:MYLAT:%d:", get_options()->MYIP, get_options()->MYLAT);
	log_debug(LD_CIRC, "@iSec:%s:CONFIG:MYLON:%d:", get_options()->MYIP, get_options()->MYLON);
	log_debug(LD_CIRC, "@iSec:%s:CONFIG:MINGAIN:%d:", get_options()->MYIP, get_options()->MINGAIN);
	...
	log_debug(LD_CIRC, "@iSec:%s:CONFIG:CONG_TIME_DEFAULT:%d:", get_options()->MYIP, get_options()->CONG_TIME_DEFAULT);
	log_debug(LD_CIRC, "@iSec:%s:CONFIG:COMBINED_WEIGHT:%d:", get_options()->MYIP, get_options()->COMBINED_WEIGHT);
	log_debug(LD_CIRC, "@iSec:%s:CONFIG:CIRC_SELECT:%d:", get_options()->MYIP, get_options()->CIRC_SELECT);
	log_debug(LD_CIRC, "@iSec:%s:CONFIG:GAMMA:%d:", get_options()->MYIP, get_options()->GAMMA);
	log_debug(LD_CIRC, "@iSec:%s:CONFIG:OPTIMAL:%d:", get_options()->MYIP, get_options()->OPTIMAL);
	log_debug(LD_CIRC, "@iSec:%s:CONFIG:VANILLA:%d:", get_options()->MYIP, get_options()->VANILLA);
	log_debug(LD_CIRC, "@iSec:%s:CONFIG:CAR:%d:", get_options()->MYIP, get_options()->CAR);
	log_debug(LD_CIRC, "@iSec:%s:CONFIG:UPDATE_S:%d:", get_options()->MYIP, get_options()->UPDATE_S);
	log_debug(LD_CIRC, "@iSec:%s:CONFIG:BUCKET:%d:", get_options()->MYIP, get_options()->BUCKET);
	log_debug(LD_CIRC, "@iSec:%s:CONFIG:CUTOFF:%d:", get_options()->MYIP, get_options()->CUTOFF);
	log_debug(LD_CIRC, "@iSec:%s:CONFIG:POWER_EXIT:%d:", get_options()->MYIP, get_options()->POWER_EXIT);
	log_debug(LD_CIRC, "@iSec:%s:CONFIG:POWER_MIDDLE:%d:", get_options()->MYIP, get_options()->POWER_MIDDLE);
	log_debug(LD_CIRC, "@iSec:%s:CONFIG:POWER_GUARD:%d:", get_options()->MYIP, get_options()->POWER_GUARD);
	log_debug(LD_CIRC, "@iSec:%s:CONFIG:POP_DEST_NUM:%d:", get_options()->MYIP, get_options()->POP_DEST_NUM);
	log_debug(LD_CIRC, "@iSec:%s:OPEN_CIRCUITS_PER_PORT_ISEC:%d:", get_options()->MYIP, get_options()->OPEN_CIRCUITS_PER_PORT_ISEC);
	log_debug(LD_CIRC, "@iSec:%s:CIRC_NUM_RANDOM_ISEC:%d:", get_options()->MYIP, get_options()->CIRC_NUM_RANDOM_ISEC);
	log_debug(LD_CIRC, "@iSec:%s:CIRC_LIFE_TIME_ISEC:%d:", get_options()->MYIP, get_options()->CIRC_LIFE_TIME_ISEC);
	log_debug(LD_CIRC, "@iSec:%s:OPEN_CIRCUITS_BOOTSTRAP_ISEC:%d:", get_options()->MYIP, get_options()->OPEN_CIRCUITS_BOOTSTRAP_ISEC);
	log_debug(LD_CIRC, "@iSec:%s:OPEN_CIRCUITS_ISEC:%d:", get_options()->MYIP, get_options()->OPEN_CIRCUITS_ISEC);
...
tor_cleanup(void)
{
  tor_log(LOG_NOTICE, LD_CONTROL,"@iSec:%s:CIRC BUILT:%d:STREAM ATTACHED:%d:Done", get_options()->MYIP, NUM_BUILT_CIRC,NUM_ATTACHED_CIRC);//iSec
  const or_options_t *options = get_options();
